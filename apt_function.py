...
from rosetta_descriptors import Get_descriptors, dG_interface
...
def apt_esm(seq, temperature=1.0):
    ...
    command = f"python3.7 test_apt_function.py --seq {seq} --temperature {temperature}"
    subprocess.run(command, stdout=subprocess.PIPE, shell=True)
...
def apt_IFA_rosetta(seq, starting_pose, scorefxn, index_ind, index_cycle):
    ...
    testepose, testedecriptors = Get_descriptors(pose = new_pose, partner1 = "CD", partner2 = "A")

    new_apt = dG_interface(pose = new_pose,to_unbind = "A_D", partner1 = "CD", partner2 = "A")
    
    testedecriptors.to_csv(f'Descriptors/temp_{index_ind}_{index_cycle}.csv')
    #new_apt = testedecriptors["ifa_dG_separated"][0] / testedecriptors["ifa_sc_value"][0]
    data = pd.DataFrame({'Sequence': [new_pose.sequence()],'dG': [new_apt]})
    data.to_csv(f'temp_{index_ind}.csv')    
...
def insert_mask(sequence, position, mask="*"):
    """
